...
arma::uvec Clump_LDD(const int nodex1, const arma::vec& nodeadjust, 
                     const arma::mat& bet1,arma::uvec& l19){
  
  Function f("Clump_LD");
  NumericVector loc_adj =  f(Named("nodex1")=nodex1, 
                             Named("nodeadjust")=nodeadjust, 
                             Named("bet1")=bet1,
                             Named("l19")=l19);
  
  arma::uvec loc_adj1 = as<arma::uvec>(loc_adj);
...
// choose IVs
void Cho_IV1(const int nodex1, const arma::vec& nodeadjust, 
             const arma::mat& bet1,arma::uvec& l19){
  ...
  l19=Clump_LDD(nodex1,nodeadjust,bet1,l18);
...
// judge edge
List Judge(const arma::mat& betaX,const arma::vec& betaY,const arma::vec& seY,
          const int use_egger,const double p_cut_step2){
  ...
  if(use_egger==1){
    ...
    arma::mat cef1 = as<arma::mat>(lm1["coefficients"]);
    coef1 = cef1(1,0);
    
    if(pval1[0]>0.05){
      List lm2=fastLm(betaX1,betaY1,false); 
      arma::vec pval2 = as<arma::vec>(lm2["pvalue"]);
      arma::mat cef2 = as<arma::mat>(lm2["coefficients"]);
      coef1 = cef1(1,0);
      
      if(pval2[0]<p_cut_step2)
        edge=1;
    }else if(pval1[1]<p_cut_step2){ 
      edge=1; 
    }
    ...
    List lm2=fastLm(betaX1,betaY1,false); 
    arma::vec pval2 = as<arma::vec>(lm2["pvalue"]);
    arma::mat cef1 = as<arma::mat>(lm2["coefficients"]);
    coef1 = cef1(0,0);
  ...
  return Rcpp::List::create(Rcpp::Named("coeff") = coef1,
...
SEXP STEP2(SEXP amattt, SEXP data_sum_beta,SEXP data_sum_se,SEXP beta1, 
           ...
           SEXP coefi,SEXP p_cut){
  
  arma::mat amatt = as<arma::mat>(amattt);
  arma::mat coeffi = as<arma::mat>(coefi);
  ...
  const double p_cut_step2=as<double>(p_cut);
        ...
        if(vary_mvmr_adjnodes==1){
          ...
          for(int k=0; k<nodesnum; k++){
            ...
            for(long e=0; e<(nl2/(k+1)); e++){
              ...
              edge_up1=Judge(betaX5,betaY3,seY3,use_egger_step2,p_cut_step2);
              ccoe = as<double>(edge_up1["coeff"]);
              edge_up = as<int>(edge_up1["edge"]);
              coeffi(cho[2*i],cho[2*i+1])=ccoe;
              if(edge_up==0){
