...
List Judge(const arma::mat& betaX,const arma::vec& betaY,const arma::vec& seY,
          const int use_egger,const double p_cut_step1){
  ...
  if(use_egger==1){
    ...
    if(pval1[0]>0.05){
      ...
      if(pval2[0]<p_cut_step1)
        edge=1;
    }else if(pval1[1]<p_cut_step1){
      edge=1;
    }
...
SEXP STEP1(SEXP data_sum_beta,SEXP data_sum_se,SEXP beta1,  
            SEXP use_eggers_step1,SEXP p_cut){
  ...
  const double p_cut_step1=as<double>(p_cut);
  ...
  for(int i=0; i<nl/2; i++){
    ...
    edge_up1=Judge(betaX1,betaY1,seY1,use_egger_step1,p_cut_step1);
    ccoe = as<double>(edge_up1["coeff"]);
    edge_up = as<int>(edge_up1["edge"]);
    coeffi(cho[2*i],cho[2*i + 1])=ccoe;
    amatt(cho[2*i],cho[2*i + 1]) = edge_up;
  ...
  for(int i=0; i<nl/2; i++){
    ...
    edge_up1=Judge(betaX2,betaY2,seY2,use_egger_step1,p_cut_step1);
    ccoe = as<double>(edge_up1["coeff"]);
    edge_up = as<int>(edge_up1["edge"]);
    coeffi(cho[2*i+1],cho[2*i])=ccoe;
    amatt(cho[2*i+1],cho[2*i]) = edge_up;
